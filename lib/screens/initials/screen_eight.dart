...
class _ScreenEightState extends State<ScreenEight> {
  ...
  Future<void> _fetchTask() async {
    try {
      ...
      if (doc.exists) {
        ...
        setState(() {
          _spanishSentence = data['spanishSentence'] ?? '';
  ...
  // FIX 1: Only the Spanish sentence card uses TTS — word bank tiles no longer
  // have individual audio buttons, so this method is only called from the card.
  void _speakSpanish() async {
    await _tts.setLanguage('es-ES');
    await _tts.speak(_spanishSentence);
  }
  ...
  void _checkAnswer() {
    // Guard: do nothing if a result sheet is already on screen
    if (_isResultSheetOpen) return;

    final isCorrect = _selectedWords.join(' ') == _correctAnswer.join(' ');

    // Haptic feedback — instant, no delay
    if (isCorrect) {
    ...
    // Play audio in background — do NOT await
    _playFeedbackSound(isCorrect);

    // Show the result sheet immediately (no delay waiting for audio)
