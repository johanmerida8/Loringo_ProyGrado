...
class _TeacherLevelScreenState extends State<TeacherLevelScreen> {
  ...
  // One mascot per unit, cycling through available animations
  ...
  Future<List<Map<String, dynamic>>> _loadGroupContent() async {
    try {
      ...
      // Fetch all units for all content docs in parallel
      ...
      for (int ci = 0; ci < contentDocs.length; ci++) {
        ...
        // Fetch lessons + unit quizzes for every unit in parallel
        ...
        for (int ui = 0; ui < unitDocs.length; ui++) {
          ...
          final unitQuizDocs = perUnitResults[ui][1].docs;

          // Fetch activities + lesson quizzes for every lesson in parallel
          final perLessonResults = await Future.wait(
            lessonDocs.map((ld) => Future.wait([
                  FirebaseFirestore.instance
                      .collection('personalizedContent')
                      .doc(contentId)
                      .collection('units')
          ...
          for (int li = 0; li < lessonDocs.length; li++) {
            ...
            for (final activityDoc in perLessonResults[li][0].docs) {
            ...
            for (final quizDoc in perLessonResults[li][1].docs) {
              final q = quizDoc.data();
              final questionIds = q['questionIds'] as List<dynamic>? ?? [];
              items.add({
                'type': 'quiz',
                'contentId': contentId,
                'unitId': unitId,
          ...
          // Unit-level quizzes
          for (final quizDoc in unitQuizDocs) {
            final q = quizDoc.data();
            final questionIds = q['questionIds'] as List<dynamic>? ?? [];
            items.add({
              'type': 'quiz',
              ...
              'lessonId': '',
              'quizId': quizDoc.id,
              'title': q['title'] ?? 'Unit Quiz',
              'quizType': q['type'] ?? 'unit_test',
              'xpReward': q['xpReward'] ?? 0,
              'questionCount': questionIds.length,
            });
          }
  ...
  Widget _buildContentHeader(Map<String, dynamic> item) {
    return Container(
  ...
  Widget _buildUnitHeader(Map<String, dynamic> item) {
    return Container(
      width: double.infinity,
  ...
  Widget _buildLessonHeader(Map<String, dynamic> item) {
    return Container(
      width: double.infinity,
      ...
      child: Row(
        children: [
          Icon(
            ...
            size: 16,
            color: greenPrimary.withOpacity(0.7),
          ),
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        title: Row(
          children: [
            Icon(
              isUnitQuiz ? Icons.assignment_turned_in : Icons.quiz,
        ...
        content: Wrap(
          ...
          children: [
            _chip(
              Icons.star,
              '${item['xpReward']} XP${isUnitQuiz ? '' : ' (practice)'}',
              Colors.amber,
            ),
            _chip(Icons.question_answer, '${item['questionCount']} questions', Colors.blue),
            _chip(
              isUnitQuiz ? Icons.grade : Icons.school,
                  ...
                  children: [
                    Text(
                      ...
                      style: const TextStyle(
                        fontSize: 14,
                        fontWeight: FontWeight.bold,
                        color: Color(0xFF4B1D96),
                      ),
                    ),
                    const SizedBox(height: 4),
                    Text(
                      '${item['questionCount']} questions · ${item['xpReward']} XP practice',
  ...
  Widget _buildUnitMascot(int unitIndex) {
    ...
    return Center(
      child: Lottie.asset(path, width: 130, height: 130),
    );
  }
  ...
  Widget build(BuildContext context) {
    final body = FutureBuilder<List<Map<String, dynamic>>>(
      ...
      builder: (context, snapshot) {
        ...
        final hasActivities =
            items.any((i) => i['type'] == 'activity' || i['type'] == 'quiz');
            ...
            return Padding(
              ...
              child: Column(
                children: [
                  ...
                  const SizedBox(height: 8),
                  Text(
                    'Assign approved content to see the activity map here',
                    textAlign: TextAlign.center,
                    style:
            ...
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  Text(
                    'Create activities for this group and they will appear here',
                    textAlign: TextAlign.center,
                    style:
        ...
        // Pre-pass: count activities per unit and map unitId → unitIndex
        ...
        // Build widget list tracking zigzag index for activities only
        int activityIndex = 0;
        ...
        for (final item in items) {
          if (item['type'] == 'content_header') {
            ...
            // Insert mascot centered among activities; skip if unit is empty
